...
import (
	...
	"github.com/bluebox-steven/souin/pkg/api"
...
func (a *adminAPI) handleAPIEndpoints(writer http.ResponseWriter, request *http.Request) error {
	if a.InternalEndpointHandlers != nil {
		a.logger.Debugw("Incoming admin API request", "request_uri", request.RequestURI)
		for k, handler := range *a.InternalEndpointHandlers.Handlers {
			a.logger.Debugw("Checking handler key", "key", k, "request_uri", request.RequestURI)
			if strings.Contains(request.RequestURI, k) {
				a.logger.Debugw("Matched handler key", "key", k, "request_uri", request.RequestURI)
